...
export default async (ctx: CliContext, options: DeplyOptions) => {
    try {
        ...
        await CommandUtil.executeCommand(ctx, CommandType.DeployCommand, CommandStage.before);
        ...
        await CommandUtil.executeCommand(ctx, CommandType.DeployCommand, CommandStage.after);
    } catch (error) {
        console.error(error);
        process.exit(-1);
    }
