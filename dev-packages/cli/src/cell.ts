...
import { program, Command } from '@celljs/cli-common/lib/command/command-protocol';
...
(async () => {
    ...
    if (CommandUtil.includesCommand(context.args, settings.compileCommands)) {
        process.send!({
            type: 'cliContext',
            data: {
                components: componentPackages.map(c => c.cellComponent),
                configHookModules,
                webpackHookModules,
                serveHookModules,
                buildHookModules,
    ...
    program
        .version(version, '-V, --version', 'version for cell')
        .option('-t, --targets [targets]', 'targets for cell', value => value ? value.split(',') : [])
        .option('-m, --mode [mode]', 'mode for cell', value => value ? value.split(',') : [])
        .option('--props-dir [propsDir]', 'props directory for cell')
        .option('--props-file [propsFile]', 'props file for cell')
        .option('--skip-auto-install [skipAutoInstall]', 'skip automatic installation of components for cell')
    ...
    program
        .command('update')
        .option('-v, --version [version]', 'version for cell')
        ...
        .description('update cell cli and current project\'s cell components')
        .action(options => {
    ...
    runtimeCmd
        ...
        .action((r, alias, options) => {
            require('@celljs/cli-runtime/lib/install/install').default({ runtime: r, alias, ...options });
        });
    runtimeCmd
        .command('use [runtime]')
        .description('use a runtime')
        .action(r => {
            require('@celljs/cli-runtime/lib/use/use').default({ runtime: r });
        });
    ...
    runtimeCmd
        ...
        .action(() => {
            require('@celljs/cli-runtime/lib/list/list').default({});
        });
    ...
    runtimeCmd
        ...
        .action(r => {
            require('@celljs/cli-runtime/lib/uninstall/uninstall').default({ runtime: r });
        });
