...
try {
    ...
    const references = getTypescriptReferences(MONOREPO, configsFolder);
    result = configureTypeScriptCompilation(MONOREPO, tsconfigCompilePath, references);
    rewriteRequired = rewriteRequired || result;

    // Configure the root `tsconfig.json` for code navigation using `tsserver`.
    const tsconfigNavPath = path.join(ROOT, 'tsconfig.json');
    result = configureTypeScriptNavigation(MONOREPO, tsconfigNavPath);
    rewriteRequired = rewriteRequired || result;
...
function configureTypeScriptNavigation(targetPackage, tsconfigPath) {
    ...
    for (const packageName of MONOREPO.workspaceDependencies) {
        ...
        if (fs.existsSync(depConfigPath) && fs.existsSync(depSrcPath)) {
            ...
            mappedFsPath = path.relative(MONOREPO.location, path.join(depSrcPath, '*'));

        } else {
            // I don't really know what to do here, simply point to our local package root.
            originalImportPath = `${packageName}/*`;
            mappedFsPath = path.relative(MONOREPO.location, path.join(depWorkspace.location, '*'));
        }
